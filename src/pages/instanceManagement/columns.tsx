...
export type InstanceRowActions = {
  ...
  onEdit: (row: InstanceRecord) => void
  onDelete?: (row: InstanceRecord) => void
  onActivate: (row: InstanceRecord) => void
  onDeactivate: (row: InstanceRecord) => void
}
...
return [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const rec = row.original
        const isActive = rec.instanceStatus?.toUpperCase() === "ACT" || rec.instanceStatus?.toUpperCase() === "ACTIVE"
